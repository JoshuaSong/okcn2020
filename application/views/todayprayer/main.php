...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		url			        : "<?php echo base_url() . 'todayprayer/data'; ?>",
		primaryField			: 'id', 
		rowNumber			: true,
                itemsPerPage			: 10,
		...
		columns					: 
                [
					{field: 'prayer_image_formatted', title: 'Image', sortable: false, width: 50, align: 'center', search: false,
	       	rowStyler: function(rowData, rowIndex) {
				return '<img src="' + rowData.prayer_image + '"  height="42">';
                  
	       		
	       		
	       		}},
		  {field: 'prayer_title', title: 'Prayer Title', editable: true, sortable:  false, width: 10, align: 'left', search: true},
                
			
				  
				  {field: 'prayer_date', title: 'Prayer Time', editable: true, sortable: false, width:10, align: 'left', search: true},
	...
	function create_update_form(rowIndex) {
		$.post("<?php echo base_url() . 'todayprayer/form'; ?>", {index : rowIndex}).done(function(data) {
	...
	function delete_action(rowIndex) {
		swal({   
			...
			$.post("<?php echo base_url() . 'todayprayer/delete'; ?>", {id : row.id}).done(function(data) {
				location.reload();
			});
		});
	}
