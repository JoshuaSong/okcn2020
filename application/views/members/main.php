	...
	<div class="content">
		<div class="panel">
			<div class="content-header no-mg-top">
				...
				<div class="content-header-title">Members</div>
			</div>
			<div class="row">
				<div class="col-md-12">
...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		url			        : "<?php echo base_url() . 'members/data'; ?>",
		primaryField			: 'id', 
		rowNumber			: true,
                itemsPerPage			: 10,
		...
		columns					: 
                [
					{field: 'user_name', title: 'UserName', editable: true, sortable:  false, width: 10, align: 'left', search: true},
		  {field: 'phone', title: 'Phone', editable: true, sortable:  false, width: 10, align: 'left', search: true},
	...
	function menu(rowData, rowIndex) {
		var menu = '<a href="javascript:;" onclick="create_message_form(' + rowIndex + ')"><i class="fa fa-envelope-open"></i> </a> ' + '&nbsp; &nbsp; &nbsp;<a href="javascript:;" onclick="main_routes(\'update\', ' + rowIndex + ')"><i class="fa fa-pencil"></i> </a> ' +
		'&nbsp; &nbsp; &nbsp; <a href="javascript:;" onclick="delete_action(' + rowIndex + ')"><i class="fa fa-trash-o"></i> </a>'
		return menu;
	}

	function create_update_form(rowIndex) {
		$.post("<?php echo base_url() . 'members/form'; ?>", {index : rowIndex}).done(function(data) {
			$('.form-panel').html(data);
		});
	}
	function create_message_form(rowIndex) {
		$('.datagrid-panel').hide();
		$('.loading-panel').fadeIn();
		$.post("<?php echo base_url() . 'members/messageform'; ?>", {index : rowIndex}).done(function(data) {
			$('.form-panel').html(data);
		});
	}
	function delete_action(rowIndex) {
		swal({   
			...
			$.post("<?php echo base_url() . 'members/delete'; ?>", {id : row.id}).done(function(data) {
				location.reload();
			});
		});
	}
