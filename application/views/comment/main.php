...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		url			        : "<?php echo base_url() . 'comment/data'; ?>",
		primaryField			: 'cm_id', 
		rowNumber			: true,
                itemsPerPage			: 10,
		...
		columns					: 
		  ...
		  {field: 'user_name', title: '이름', editable: true, sortable:  false, width: 10, align: 'left', search: true},
                
				  {field: 'contents', title: '내용', editable: true, sortable: false, width:100, align: 'left', search: true},
				  
				  {field: 'create_time', title: '시간', editable: true, sortable: false, width:10, align: 'left', search: true},
	...
	function create_update_form(rowIndex) {
		$.post("<?php echo base_url() . 'comment/form'; ?>", {index : rowIndex}).done(function(data) {
	...
	function delete_action(rowIndex) {
		swal({   
			...
			$.post("<?php echo base_url() . 'comment/delete'; ?>", {cm_id : row.cm_id}).done(function(data) {
				location.reload();
			});
		});
	}
