...
var datagrid = $("#datagrid").datagrid({
		...
		columns					: [
	       	...
	       	rowStyler: function(rowData, rowIndex) {
				return '<img src="' + rowData.banners_image + '"  height="42">';
...
$('#mySelect').change(function(){ 
	...
	function create_update_form(rowIndex) {
		$.post("<?php echo base_url() . 'slider/form/'; ?>", {index : rowIndex}).done(function(data) {
	...
	function delete_action(rowIndex) {
		swal({   
			...
			$.post("<?php echo base_url() . 'slider/delete'; ?>", {id : row.id}).done(function(data) {
				datagrid.reload();
			});
		});
	}
