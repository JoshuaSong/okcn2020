		...
		var filepicker = {
            ...
            sort_files: function() {
            	$('#' + filepicker.wrapper).find(".uploaderbox").each(function() {
            		...
            		var placeholderX, placeholderY
            		var cursorPositionX = null, cursorPositionY = null
                        if(config.file_to_content)
                        {
                        var tocontents = $(box).find(".uploaderbox-tocontents")
                         $(tocontents).off()
                        $(tocontents).click(function() {
            			var filename = $(box).find('.uploaderbox-text').text()
            			// bg_url =  bg_url.replace(/.*\s?url\([\'\"]?/, '').replace(/[\'\"]?\).*/, '')
                               //  alert(filename)
            			 var img ="<p><img style='width:100%' src='/"+filename+"'  /></p>"
            			...
            			var $iframe = $('.cke_wysiwyg_frame')
            			 ...
            			 $iframe.contents().find("body").append(img)
                              ...
                              $(box).find(".uploaderbox-tocontents").hide()
            		$(box).off()
	        	...
	        	$(template).on('click', function() {
    				...
    				if (validate_maximum_file_size && validate_file_type) {
				    	...
				    	$(this).find('.uploaderbox-close').show()
                                         $(this).find('.uploaderbox-delete').hide()
			...
			filepicker_box_template: function(file) {
				...
				!selected ? $(template).find('.uploaderbox-close').hide() : ''
                                selected ? $(template).find('.uploaderbox-delete').hide() : ''
